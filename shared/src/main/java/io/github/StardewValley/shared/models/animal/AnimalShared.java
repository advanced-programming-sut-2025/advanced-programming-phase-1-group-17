...
public class AnimalShared implements Placeable {
    private String name;
    ...
    public AnimalShared(String name, AnimalType animalType, AnimalPlace animalPlace){
...
//            ,(int)animalPlace.getY());
    ...
    public void update(float delta) {
        ...
        if (followingPath && currentPathIndex < path.size()) {
            ...
            if (directionVec.len() < 2f) { // رسیدیم به تایل مقصد فعلی
                ...
                if (Math.abs(directionVec.x) > Math.abs(directionVec.y)) {
                    direction = directionVec.x > 0 ? Right : Direction.Left;
                } else {
                    direction = directionVec.y > 0 ? Direction.Up : Down;
                }
            }
        }
        ...
        if(true) {
            ...
            //TODO
//            Texture backgroundTexture = GameAssetManager
